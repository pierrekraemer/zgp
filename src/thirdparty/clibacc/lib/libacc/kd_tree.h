...
private:
    std::vector<std::array<float, K>> vertices;
    struct Node
    {
        typedef IdxType ID;
        IdxType vertex_id;
...
public:
    ...
    static Ptr create(std::vector<std::array<float, K>> const &vertices,
                      int max_threads = std::thread::hardware_concurrency())
    {
        return std::make_shared<KDTree>(vertices, max_threads);
    }

    KDTree(std::vector<std::array<float, K>> const &vertices,
           int max_threads = std::thread::hardware_concurrency());
    KDTree(std::array<float, K> const *vertices,
           IdxType nb_vertices,
           int max_threads = std::thread::hardware_concurrency());

    std::array<float, K> const &vertex(IdxType vertex_id) const
    {
        return vertices[vertex_id];
    }

    bool
    find_nn(std::array<float, K> point, std::pair<IdxType, float> *nn,
            float max_dist = std::numeric_limits<float>::infinity()) const;

    bool
    find_nns(std::array<float, K> point, std::size_t n,
...
template <unsigned K, typename IdxType>
KDTree<K, IdxType>::KDTree(std::vector<std::array<float, K>> const &vertices,
                           int max_threads)
...
template <unsigned K, typename IdxType>
KDTree<K, IdxType>::KDTree(std::array<float, K> const *vertices_array, IdxType nb_vertices,
                           int max_threads)
    : num_nodes(0)
{
    vertices.reserve(nb_vertices);
    for (IdxType i = 0; i < nb_vertices; ++i)
...
template <unsigned K, typename IdxType>
bool KDTree<K, IdxType>::find_nn(std::array<float, K> point,
...
template <unsigned K, typename IdxType>
bool KDTree<K, IdxType>::find_nns(std::array<float, K> vertex, std::size_t n,
                                  std::vector<std::pair<IdxType, float>> *nns_ptr, float max_dist) const
{
    ...
    while (true)
    {
        ...
        if (down)
        {
            ...
            {
                float delta = vertex[i] - vertices[node.vertex_id][i];
