...
template <typename Vec3fType>
inline float volume(AABB<Vec3fType> const &aabb)
{
 ...
 * on 23rd March 2015 */
template <typename Vec3fType>
inline bool intersect_box(Ray<Vec3fType> const &ray, AABB<Vec3fType> const &aabb, float *tmin_ptr)
{
...
template <typename Vec3fType>
inline bool intersect_tri(Ray<Vec3fType> const &ray, Tri<Vec3fType> const &tri,
...
template <typename Vec3fType>
inline Vec3fType closest_point_box(Vec3fType const &v, AABB<Vec3fType> const &aabb)
{
    Vec3fType ret;
    for (int i = 0; i < 3; ++i)
    {
...
template <typename Vec3fType>
inline Vec3fType closest_point_tri(Vec3fType const &vertex, Tri<Vec3fType> const &tri, Vec3fType *bcoords_ptr)
{
    Vec3fType ab = tri.b - tri.a;
    Vec3fType ac = tri.c - tri.a;
    Vec3fType normal = ab.cross(ac);
    ...
    if (bcoords[0] < 0.0f)
    {
        ...
        float n = bc.norm();
        float t = std::max(0.0f, std::min(bc.dot(p - tri.b) / n, n)) / n;
        *bcoords_ptr = Vec3fType(0.0f, 1.0f - t, t);
        return tri.b + t * bc;
    }
    if (bcoords[1] < 0.0f)
    {
        ...
        float n = ca.norm();
        float t = std::max(0.0f, std::min(ca.dot(p - tri.c) / n, n)) / n;
        *bcoords_ptr = Vec3fType(t, 0.0f, 1.0f - t);
        return tri.c + t * ca;
    }
    if (bcoords[2] < 0.0f)
    {
        // Vec3fType ab = tri.b - tri.a;
        float n = ab.norm();
        float t = std::max(0.0f, std::min(ab.dot(p - tri.a) / n, n)) / n;
        *bcoords_ptr = Vec3fType(1.0f - t, t, 0.0f);
        return tri.a + t * ab;
    }
