...
private:
    ...
    void closest_point(Vec3fType const &vertex, typename Node::ID node_id, Vec3fType &cp, IdxType &idx, Vec3fType &bcoords) const;

public:
    static Ptr create(std::vector<IdxType> const &faces,
    ...
    bool closest_point(Vec3fType const &vertex, Vec3fType &cp, IdxType &idx, Vec3fType &bcoords) const;
};
                                    ...
                                    std::vector<AABB> const &aabbs)
{
    ...
    for (char d = 0; d < 3; ++d)
    {
        ...
        {
            std::size_t nr = n - nl;
            float cost = 1.2f + (surface_area(left_aabb) / surface_area(node.aabb) * nl + surface_area(right_aabbs[idx]) / surface_area(node.aabb) * nr);
            if (cost <= min_cost)
            {
                min_cost = cost;
...
BVHTree<IdxType, Vec3fType>::ssplit(typename Node::ID node_id, std::vector<AABB> const &aabbs)
{
    ...
    for (char d = 0; d < 3; ++d)
    {
        std::sort(&indices[node.first], &indices[node.last],
                  ...
                  {
                      return mid(aabbs[first], d) < mid(aabbs[second], d) || (mid(aabbs[first], d) == mid(aabbs[second], d) && first < second);
                  });
        ...
        for (IdxType i = node.first + 1; i < node.last; ++i)
        {
            ...
            IdxType nr = n - nl;
            float cost = 1.2f + (surface_area(left_aabb) / surface_area(node.aabb) * nl + surface_area(right_aabbs[nl]) / surface_area(node.aabb) * nr);
            if (cost <= min_cost)
            {
                min_cost = cost;
...
bool BVHTree<IdxType, Vec3fType>::intersect(Ray const &ray, typename Node::ID node_id, Hit &hit) const
{
    ...
    for (std::size_t i = node.first; i < node.last; ++i)
    {
        float t;
        Vec3fType bcoords;
        if (acc::intersect_tri(ray, tris[i], &t, &bcoords))
        {
            if (t > hit.t)
                continue;
            hit.idx = indices[i];
...
template <typename IdxType, typename Vec3fType>
bool BVHTree<IdxType, Vec3fType>::intersect(Ray const &ray, Hit &hit) const
{
    hit.t = inf;
    ...
    while (true)
    {
        ...
        if (node.left != NAI && node.right != NAI)
        {
            ...
            bool left = acc::intersect_box(ray, nodes[node.left].aabb, &tmin_left);
            bool right = acc::intersect_box(ray, nodes[node.right].aabb, &tmin_right);
            if (left && right)
            {
...
template <typename IdxType, typename Vec3fType>
void BVHTree<IdxType, Vec3fType>::closest_point(Vec3fType const &vertex,
                                                typename Node::ID node_id, Vec3fType &cp, IdxType &idx, Vec3fType &bcoords) const
{
    Node const &node = nodes[node_id];
    ...
    for (std::size_t i = node.first; i < node.last; ++i)
    {
        Vec3fType cp_tri_bcoords;
        Vec3fType cp_tri = acc::closest_point_tri(vertex, tris[i], &cp_tri_bcoords);
        ...
        if (dist_tri < dist)
        {
            ...
            cp = cp_tri;
            idx = indices[i];
            bcoords = cp_tri_bcoords;
        }
    }
}

template <typename IdxType, typename Vec3fType>
bool BVHTree<IdxType, Vec3fType>::closest_point(Vec3fType const &vertex, Vec3fType &cp, IdxType &idx, Vec3fType &bcoords) const
{
    float max_dist = inf;
    float dist_squared = max_dist * max_dist;
    ...
    while (true)
    {
        ...
        if (node.left != NAI && node.right != NAI)
        {
            Vec3fType cp_left = acc::closest_point_box(vertex, nodes[node.left].aabb);
            Vec3fType cp_right = acc::closest_point_box(vertex, nodes[node.right].aabb);
            float dmin_left_squared = (cp_left - vertex).square_norm();
            float dmin_right_squared = (cp_right - vertex).square_norm();
            bool left = dmin_left_squared < dist_squared;
            bool right = dmin_right_squared < dist_squared;
            if (left && right)
            {
                if (dmin_left_squared < dmin_right_squared)
                {
                    s.push(node.right);
                    node_id = node.left;
                }
        ...
        else
        {
            Vec3fType cp_leaf;
            IdxType idx_leaf;
            Vec3fType bcoords_leaf;
            closest_point(vertex, node_id, cp_leaf, idx_leaf, bcoords_leaf);
            float dist_leaf_squared = (cp_leaf - vertex).square_norm();
            if (dist_leaf_squared < dist_squared)
            {
                dist_squared = dist_leaf_squared;
                cp = cp_leaf;
                idx = idx_leaf;
                bcoords = bcoords_leaf;
            }
