...
pub fn computeCornerAngles(
    ...
    while (it.next()) |corner| {
        corner_angle.valuePtr(corner).* = cornerAngle(surface_mesh, vertex_position, corner);
    }
}
...
pub fn edgeDihedralAngle(
    ...
    const d = edge.dart();
    assert(!surface_mesh.isBoundaryDart(d));
...
pub fn computeEdgeDihedralAngles(
    ...
    defer it.deinit();
    while (it.next()) |edge| {
        edge_dihedral_angle.valuePtr(edge).* = edgeDihedralAngle(surface_mesh, vertex_position, edge);
    }
}
