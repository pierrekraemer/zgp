...
const StdDataComputation = struct {
    ...
    fn ComputeFuncType(comptime self: *const StdDataComputation) type {
        ...
        params[1] = .{ .is_generic = false, .is_noalias = false, .type = *IncidenceGraph };
        inline for (self.reads, 0..self.reads.len) |read_tag, i| {
            params[i + 2] = .{
                .is_generic = false,
                .is_noalias = false,
                .type = @typeInfo(@FieldType(IncidenceGraphStore.IncidenceGraphStdDatas, @tagName(read_tag))).optional.child,
            };
        }
        params[nbparams - 1] = .{
            .is_generic = false,
            .is_noalias = false,
            .type = @typeInfo(@FieldType(IncidenceGraphStore.IncidenceGraphStdDatas, @tagName(self.computes))).optional.child,
        };
        return @Type(.{ .@"fn" = .{
