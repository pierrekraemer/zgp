...
void main() {
  ...
  basisX -= dot(basisX, cylDir) * cylDir;
  if (abs(basisX.x) < 0.1) {
    basisX = vec3(0., 1., 0.);
    basisX -= dot(basisX, cylDir) * cylDir;
  }
  basisX = normalize(basisX);
  vec3 basisY = normalize(cross(cylDir, basisX));

  vec4 dx = vec4(basisX * u_cylinder_radius, 0.);
  vec4 dy = vec4(basisY * u_cylinder_radius, 0.);
  ...
  vec4 p5 = gl_in[1].gl_Position - dx - dy;
  vec4 p6 = gl_in[1].gl_Position + dx - dy;
  vec4 p7 = gl_in[1].gl_Position - dx + dy;
  vec4 p8 = gl_in[1].gl_Position + dx + dy;
  
  vec4 v0_proj = u_projection_matrix * gl_in[0].gl_Position;
  vec4 v1_proj = u_projection_matrix * gl_in[1].gl_Position;
